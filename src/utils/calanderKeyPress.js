...
export default function useCalendarNavigation(ref, isOpen, onSelect) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      // If nothing selected yet, default to first enabled tile
      if (activeDate === null) {
      ...
      if (e.key === "ArrowDown") newIndex = Math.min(activeDate + 7, tiles.length - 1); // jump by week
      if (e.key === "ArrowUp") newIndex = Math.max(activeDate - 7, 0);

      if (e.key === "Enter" || e.key === " ") {
        tiles[activeDate]?.click(); // simulate click
